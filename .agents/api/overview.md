...
`src/configs/conf.tsx` resolves `VITE_ENV` to `DEV`, `TEST`, or `PROD`, defaulting to `DEV` for missing or unknown values. It selects `VITE_URL_API` for `PROD`, `VITE_URL_API_TEST` for `TEST` when available, falls back to `VITE_URL_API_DEV` for `TEST`, and uses `VITE_URL_API_DEV` for `DEV`. Environment values are intentionally not copied here. The tracking page trims and lowercases the submitted email before calling the tracking endpoint.
...
| GET | `/vetrun/tracking?email=<email>` | `FunctionGetTrackingAll`; sends the normalized order email as a query parameter |
...
Exact response schemas, HTTP status matrix, authentication, rate limits, server-side validation, and backend encryption-key handling are not defined in this repository. `ApiResponse.data` is typed as `any`; participant and tracking lookup UIs accept either one response object or an array.
